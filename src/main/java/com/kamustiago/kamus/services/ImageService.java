...
import com.kamustiago.kamus.services.exceptions.FileException;
...
public class ImageService {
	...
	public BufferedImage getJpgImageFromFile(MultipartFile uploadedFile) {
		...
		if(!"png".equals(ext) && !"jpg".equals(ext)) {
			throw new FileException("Somenete iamgens PNG e JPG são permitidas");
		}
		try {
	...
	public BufferedImage pngToJpg(BufferedImage img) {
		...
		jpgImage.createGraphics().drawImage(img, 0, 0, Color.WHITE, null);// Utilizo o color white pq algumas imagens png tem fundo
		return jpgImage;																 // transparente e sera preeenchida com branco	
	}
